...
trait Reactive[+A, +N <: ReactiveNotification[A]] {
  ...
  protected[reactive] def addDependant(maybeTransaction: Option[Transaction], dependant: ReactiveDependant[N])
  protected[reactive] def maybeNotification(transaction: Transaction): Option[N]
