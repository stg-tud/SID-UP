...
class SignalMonadicTest extends FunSuite {
  test("explicit usage: changing inner and outer values") {
    val sx = Var(1)
    val sy = Var(2)
    val ssumxy = atomic { tx => sx.transactional.tflatMap { (x, itx) => sy.transactional.map { y => x + y }(itx) }(tx) }
    val ssumyx = atomic { tx => sy.transactional.tflatMap { (y, itx) => sx.transactional.map { x => x + y }(itx) }(tx) }
  ...
  test("explicit usage: observing flattened values") {
    ...
    val ssum = atomic { tx =>
      sx.transactional.tflatMap { (x, itx) => sy.transactional.map { y => x + y }(itx) }(tx)
    }
