...
class FlattenSignalTest extends FunSuite {
  test("inner value update works") {
    ...
    assertResult(Set(inner.uuid, outer.uuid)) { flattened.singleSourceDependencies }

    inner << 234
    assertResult(234) { flattened.now }
    assertResult(Set(inner.uuid, outer.uuid)) { flattened.singleSourceDependencies }
  ...
  test("outer value update works") {
    ...
    outer << inner3
    assertResult(234) { flattened.now }
    assertResult(Set(inner3.uuid, outer.uuid)) { flattened.singleSourceDependencies }
    assertResult(1) { log.size }
    val notification2 = log.dequeue()
    assertResult(false) { notification2.valueChanged }
    assertResult(234) { notification2.newValue }
    ...
    assertResult(444) { flattened.now }
    assertResult(Set(inner3.uuid, outer.uuid)) { flattened.singleSourceDependencies }
    assertResult(1) { log.size }
    val notification3 = log.dequeue()
    assertResult(true) { notification3.valueChanged }
  ...
  test("double nested") {
    ...
    assertResult(Set(inner.uuid, middle.uuid, outer1.uuid)) { flattened1.singleSourceDependencies }
    assertResult(1) { flattened2.now }
    assertResult(Set(inner.uuid, middle.uuid, outer2.uuid)) { flattened2.singleSourceDependencies }
    ...
    assertResult(Set(middle.uuid, outer1.uuid)) { flattened1.singleSourceDependencies }
    assertResult(1) { log1.size }
    ...
    assertResult(Set(middle.uuid, outer2.uuid)) { flattened2.singleSourceDependencies }
    assertResult(1) { log2.size }
    ...
    assertResult(Set(inner2.uuid, middle2.uuid, outer1.uuid)) { flattened1.singleSourceDependencies }
    assertResult(1) { log1.size }
    ...
    assertResult(Set(inner2.uuid, middle2.uuid, outer2.uuid)) { flattened2.singleSourceDependencies }
    assertResult(1) { log2.size }
