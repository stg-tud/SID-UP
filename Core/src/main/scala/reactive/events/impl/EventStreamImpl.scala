...
trait EventStreamImpl[A] extends ReactiveImpl[A, A] with EventStream[A] {
  ...
  override def map[B](op: A => B)(implicit inTxn: InTxn): EventStream[B] = new MappedEventStream(this, op);
  override def collect[B](op: PartialFunction[A, B])(implicit inTxn: InTxn): EventStream[B] = new PartiallyMappedEventStream(this, op);
  override def merge[B >: A](streams: EventStream[B]*)(implicit inTxn: InTxn): EventStream[B] = new MergeStream(this :: streams.toList);
  override def fold[B](initialValue: B)(op: (B, A) => B)(implicit inTxn: InTxn): Signal[B] = new FoldSignal(initialValue, this, op);
