...
trait EventStreamImpl[A] extends ReactiveImpl[A, A] with EventStream[A] {
  ...
  override def map[B](op: A => B)(implicit inTxn: InTxn): EventStream[B] = new TransformEventStream[B, A](this, _.map(op), inTxn)
  override def collect[B](op: PartialFunction[A, B])(implicit inTxn: InTxn): EventStream[B] = new TransformEventStream[B, A](this, _.collect(op), inTxn)
  override def filter(op: A => Boolean)(implicit inTxn: InTxn): EventStream[A] = new TransformEventStream[A, A](this, _.filter(op), inTxn)
