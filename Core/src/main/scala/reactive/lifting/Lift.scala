...
object Lift {
  ...
  implicit def signal2[A1, A2, B](fun: (A1, A2) => B): (Signal[A1], Signal[A2]) => Signal[B] = (a1, a2) => new FunctionalSignal({ t => fun(a1.value(t), a2.value(t)) }, Set(a1, a2))
  implicit def signal3[A1, A2, A3, B](fun: (A1, A2, A3) => B): (Signal[A1], Signal[A2], Signal[A3]) => Signal[B] = (a1, a2, a3) => new FunctionalSignal({ t => fun(a1.value(t), a2.value(t), a3.value(t)) }, Set(a1, a2, a3))

  implicit def signalSink1[A](fun: A => Unit): Signal[A] => Unit = a => { fun(a.now); a.observe(fun) }
  implicit def signalSink2[A1, A2](fun: (A1, A2) => Unit): (Signal[A1], Signal[A2]) => Unit = (a1, a2) => signalSink1(fun.tupled)(new FunctionalSignal({ t => (a1.value(t), a2.value(t)) }, Set(a1, a2)))
  implicit def signalSink3[A1, A2, A3](fun: (A1, A2, A3) => Unit): (Signal[A1], Signal[A2], Signal[A3]) => Unit = (a1, a2, a3) => signalSink1(fun.tupled)(new FunctionalSignal({ implicit t => (a1.value(t), a2.value(t), a3.value(t)) }, Set(a1, a2, a3)))
