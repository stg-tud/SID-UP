...
object Lift {
  object single {
    ...
    implicit def signal2[A1, A2, B](fun: (A1, A2) => B): (Signal[A1], Signal[A2]) => Signal[B] = (a1, a2) => new FunctionalSignal({ t => fun(a1.now(t), a2.now(t)) }, a1, a2)
    implicit def signal3[A1, A2, A3, B](fun: (A1, A2, A3) => B): (Signal[A1], Signal[A2], Signal[A3]) => Signal[B] = (a1, a2, a3) => new FunctionalSignal({ t => fun(a1.now(t), a2.now(t), a3.now(t)) }, a1, a2, a3)

    implicit def signalSink1[A](fun: A => Unit): Signal[A] => Unit = a => atomic { transactional.signalSink1(fun)(a, _) }
    implicit def signalSink2[A1, A2](fun: (A1, A2) => Unit): (Signal[A1], Signal[A2]) => Unit = (a1, a2) => atomic { transactional.signalSink1(fun.tupled)(new FunctionalSignal({ t => (a1.now(t), a2.now(t)) }, a1, a2), _) }
    implicit def signalSink3[A1, A2, A3](fun: (A1, A2, A3) => Unit): (Signal[A1], Signal[A2], Signal[A3]) => Unit = (a1, a2, a3) => atomic { transactional.signalSink1(fun.tupled)(new FunctionalSignal({ implicit t => (a1.now(t), a2.now(t), a3.now(t)) }, a1, a2, a3), _) }
  ...
  object transactional {
    ...
    implicit def signal2[A1, A2, B](fun: (A1, A2) => B): (Signal[A1], Signal[A2], InTxn) => Signal[B] = (a1, a2, inTxn) => new FunctionalSignal({ t => fun(a1.now(t), a2.now(t)) }, a1, a2)
    implicit def signal3[A1, A2, A3, B](fun: (A1, A2, A3) => B): (Signal[A1], Signal[A2], Signal[A3], InTxn) => Signal[B] = (a1, a2, a3, inTxn) => new FunctionalSignal({ t => fun(a1.now(t), a2.now(t), a3.now(t)) }, a1, a2, a3)

    implicit def signalSink1[A](fun: A => Unit): (Signal[A], InTxn) => Unit = (a, inTxn) => { fun(a.now(inTxn)); a.observe(fun)(inTxn) }
    implicit def signalSink2[A1, A2](fun: (A1, A2) => Unit): (Signal[A1], Signal[A2], InTxn) => Unit = (a1, a2, inTxn) => signalSink1(fun.tupled)(new FunctionalSignal({ t => (a1.now(t), a2.now(t)) }, a1, a2), inTxn)
    implicit def signalSink3[A1, A2, A3](fun: (A1, A2, A3) => Unit): (Signal[A1], Signal[A2], Signal[A3], InTxn) => Unit = (a1, a2, a3, inTxn) => signalSink1(fun.tupled)(new FunctionalSignal({ implicit t => (a1.now(t), a2.now(t), a3.now(t)) }, a1, a2, a3), inTxn)
