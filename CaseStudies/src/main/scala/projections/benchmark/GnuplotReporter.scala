...
class GnuplotReporter extends Reporter {
  ...
  def formatHeader(cd: CurveData) = (cd.measurements.head.params.axisData.keys.map { key => s"param-$key" }.toSeq :+ "value").mkString(delimiter)
  ...
  def formatBody(cd: CurveData) = {
    ...
    def groupFormatMeasurements(xs: Seq[Seq[Any]]): Seq[String] =
      if (xs.isEmpty) Seq()
        ...
        val (firstGroup, rest) = xs.span(_(0) == xs(0)(0))
